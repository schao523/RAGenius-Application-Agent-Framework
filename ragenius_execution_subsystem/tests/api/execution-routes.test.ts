...
test("async Agent submission resolves governance before it can enter the queue", async () => {
  const selectionService = {
    async resolve() {
      throw new AgentSkillSelectionError(
        "AGENT_SKILL_NOT_BOUND",
        "Agent skill binding is revoked."
      );
    }
  ...
  const app = buildApp({ agentSkillSelectionService: selectionService }, runtimeConfig);
  await app.ready();
  ...
  assert.equal(response.statusCode, 409);
  assert.equal(response.json().error.code, "AGENT_SKILL_NOT_BOUND");
  await app.close();
});
