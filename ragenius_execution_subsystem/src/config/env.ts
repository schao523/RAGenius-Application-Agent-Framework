...
const envSchema = z.object({
  ...
  CODEX_AGENT_SKILL_MAX_DEPTH: z.coerce.number().int().nonnegative().default(12),
  ...
  AGENT_INPUT_ALLOWED_MIME_TYPES: z.string().default(
    "video/mp4,application/pdf,application/vnd.openxmlformats-officedocument.wordprocessingml.document,text/plain,text/markdown,application/octet-stream"
  ),
  ...
  CODEX_APP_SERVER_INTERACTIVE_ENABLED: booleanEnv(false),
  CODEX_APP_SERVER_COMMAND: z.string().trim().default("codex"),
  ...
  CODEX_INTERACTIVE_AUTH_HANDOFF_ENABLED: booleanEnv(false),
  CODEX_INTERACTIVE_USER_ACTION_ENABLED: booleanEnv(false),
  CODEX_MCP_AUTH_ALLOWED_HOSTS_JSON: z.string().default("[]"),
  CODEX_MANAGED_AUTH_TARGETS_JSON: z.string().default("[]"),
  ...
  OPENCLAW_GATEWAY_CHAT_IDLE_TTL_MS: z.coerce.number().int().positive().default(900000),
  ...
  OPENCLAW_GATEWAY_RPC_TIMEOUT_MS: z.coerce.number().int().positive().default(15000),
  OPENCLAW_GATEWAY_INTERACTION_TTL_MS: z.coerce.number().int().positive().default(900000),
  OPENCLAW_GATEWAY_MAX_MESSAGE_BYTES: z.coerce.number().int().positive().default(1048576),
  OPENCLAW_GATEWAY_RECONNECT_MAX_ATTEMPTS: z.coerce.number().int().positive().default(5),
  OPENCLAW_GATEWAY_RECONNECT_BASE_DELAY_MS: z.coerce.number().int().positive().default(250),
